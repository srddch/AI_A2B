...
def get_unique_bidirectional_edges(graph_edges):
    ...
    for from_node, to_node in graph_edges:
        edge_key = tuple(sorted([str(from_node), str(to_node)]))

        if edge_key not in seen_edges:
            visual_edges.append((from_node, to_node))
    ...
    if mode == "best" and best_path:
        ...
        for route in paths:
            ...
            for from_node, to_node in route_edges:
                edges_to_draw.add((from_node, to_node))
                edges_to_draw.add((to_node, from_node))
